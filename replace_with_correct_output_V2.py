...
def normalize_numeric_value(value):
    """
    Normalize a value for comparison, handling numeric type conversions.
    Returns (normalized_value, original_type) tuple.
    """
    if value is None:
    ...
    if isinstance(value, (int, float)):
        return float(value), original_type
    elif isinstance(value, str):
        try:
            # Try int first
            num_val = int(value)
            return float(num_val), original_type
        except ValueError:
            try:
                # Try float
                num_val = float(value)
                return num_val, original_type
            except ValueError:
...
def values_are_equivalent(val1, val2):
    """
    Check if two values are equivalent, considering numeric type conversions.
    "200" == 200 == 200.0 -> True
    """
    if val1 == val2:
        return True
    
    norm1, _ = normalize_numeric_value(val1)
    norm2, _ = normalize_numeric_value(val2)
    
    # If both are numeric, compare as floats
    if isinstance(norm1, (int, float)) and isinstance(norm2, (int, float)):
        return abs(float(norm1) - float(norm2)) < 1e-9
...
def deep_compare_and_track(old_data, new_data, path="", changes=None):
    ...
    Handles numeric type conversions (e.g., "200" -> 200.0).
    """
    if changes is None:
        changes = {}
    ...
    if type(old_data) != type(new_data):
        if old_data is not None and new_data is not None:
            # Check if they're equivalent despite type difference
            if not values_are_equivalent(old_data, new_data):
            ...
            # Even if equivalent, store mapping for numeric conversions
            else:
                changes[str(old_data)] = str(new_data)
                if not isinstance(old_data, str):
                    changes[old_data] = new_data
    ...
    if isinstance(old_data, dict) and isinstance(new_data, dict):
        ...
        # Scalar values - track if changed (considering numeric equivalence)
        if old_data != new_data:
            if not values_are_equivalent(old_data, new_data):
                if old_data is not None and new_data is not None:
                    ...
                    if not isinstance(old_data, str):
                        changes[old_data] = new_data
            # Even if equivalent, track numeric type changes
...
def update_outputs_array(outputs, value_mapping):
    ...
    Handles both string and typed values, including numeric conversions.
    "200" matches with 200.0, "120" matches with 120.0, etc.
    ...
    for idx, output_val in enumerate(outputs):
        ...
        # Strategy 1: Exact match (preserves types like int vs string)
        ...
        # Strategy 3: Numeric equivalence match
        # Check if output_val is numerically equivalent to any key in value_mapping
        else:
            norm_output, output_type = normalize_numeric_value(output_val)
            if isinstance(norm_output, (int, float)):
                for map_key, map_val in value_mapping.items():
                    if map_val is None:
                        continue
                    norm_key, _ = normalize_numeric_value(map_key)
                    if isinstance(norm_key, (int, float)):
                        # Check numeric equivalence
                        if abs(float(norm_output) - float(norm_key)) < 1e-9:
                            new_val = map_val
                            found_replacement = True
                            modifications.append(f"  Updated outputs[{idx}]: {output_val} -> {new_val} (numeric match: {map_key})")
                            break
        
        if found_replacement and new_val is not None:
            # Try to preserve the original type of the output value
